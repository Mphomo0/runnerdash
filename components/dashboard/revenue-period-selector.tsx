...
import { DollarSign, ShoppingCart, Package, AlertCircle, ChevronDown, ChevronRight } from "lucide-react"
...
interface RevenuePeriodSelectorProps {
  weeklyRevenue: { [week: string]: { total: number; runner: number; orders: number } }
  monthlyRevenue: { [month: string]: { total: number; runner: number; orders: number } }
  yearlyRevenue: { [year: string]: { total: number; runner: number; orders: number } }
  weeklyOrders: { [week: string]: { total: number; completed: number; pending: number; pendingPayment: number } }
  monthlyOrders: { [month: string]: { total: number; completed: number; pending: number; pendingPayment: number } }
  yearlyOrders: { [year: string]: { total: number; completed: number; pending: number; pendingPayment: number } }
}
...
export function RevenuePeriodSelector({
  ...
  const getPeriodData = (key: string): PeriodData => {
    ...
    if (period === "weekly") {
      ...
      const orders = weeklyOrders[key] || { total: 0, completed: 0, pending: 0, pendingPayment: 0 }
      ...
      const orders = monthlyOrders[key] || { total: 0, completed: 0, pending: 0, pendingPayment: 0 }
      activeOrders = orders.pending
      completedOrders = orders.completed
      pendingPayments = orders.pendingPayment
    } else {
      revData = yearlyRevenue[key] || { total: 0, runner: 0, orders: 0 }
      const orders = yearlyOrders[key] || { total: 0, completed: 0, pending: 0, pendingPayment: 0 }
      activeOrders = orders.pending
      completedOrders = orders.completed
      pendingPayments = orders.pendingPayment
    }
  ...
  const currentKey = selectedKey || periodKeys[0] || ""
  ...
  return (
    ...
    >
      ...
      <div
        ...
        style={{
          background: 'linear-gradient(to bottom, rgba(13, 20, 32, 0.8), rgba(8, 12, 20, 0.9))',
      ...
      >
        <div className="space-y-4">
          <div className="flex items-center gap-2">
            ...
            <span className="text-[10px] font-bold tracking-[0.2em] uppercase" style={{ color: 'var(--text-muted)', fontFamily: 'var(--font-mono)' }}>
              Analysis Period
            </span>
          </div>
          <div className="flex gap-1 p-1 rounded-sm" style={{ backgroundColor: 'rgba(255, 255, 255, 0.03)', border: '1px solid var(--border-subtle)' }}>
        ...
        <div className="relative group max-w-xs w-full">
          <label className="block text-[10px] font-bold tracking-[0.15em] uppercase mb-2 ml-1" style={{ color: 'var(--text-muted)', fontFamily: 'var(--font-mono)' }}>
          ...
          <div className="relative transition-all duration-300 focus-within:ring-1 focus-within:ring-[var(--accent)]/50">
            ...
            >
              {periodKeys.length === 0 ? (
                ...
                periodKeys.map((key) => (
                  <option key={key} value={key} style={{ backgroundColor: '#080C14' }}>
                    {period === "weekly" ? key : period === "monthly" ? formatMonthKey(key) : key}
                  </option>
                ))
              )}
            </select>
          ...
          >
            ...
            <div className="flex items-start justify-between mb-6">
              <div className="space-y-1">
                ...
                <p className="text-[11px] font-medium leading-tight max-w-[140px] opacity-70 group-hover:opacity-100 transition-opacity" style={{ color: 'var(--text-muted)' }}>
                  {stat.subtext}
                </p>
              </div>
              <div
                ...
                style={{ backgroundColor: `${stat.accentColor}12`, color: stat.accentColor, border: `1px solid ${stat.accentColor}20` }}
              >
