...
const getFeeForQuantity = (tiers: any[], quantity: number) => {
    ...
    const tier = tiers.find((t: any) => quantity >= t.minItems && quantity <= t.maxItems);
    if (tier) return tier.feeAmount;

    // If not found in explicit tiers, check if it exceeds the highest tier
    const sortedTiers = [...tiers].sort((a: any, b: any) => b.maxItems - a.maxItems);
...
export const getDashboardStats = query({
    ...
    handler: async (ctx) => {
        ...
        const pendingPayments = orders.filter(o => !o.itemsPaid || !o.runnerFeePaid).length +
            runnerOrders.filter(o => !o.itemsPaid || !o.runnerFeePaid).length;
        const ordersToBuy = orders.filter(o => o.status === "PENDING").length +
        ...
        const weeklyRevenue: { [week: string]: { total: number, runner: number, orders: number } } = {
        ...
        const monthlyRevenue: { [month: string]: { total: number, runner: number, orders: number } } = {};
        const yearlyRevenue: { [year: string]: { total: number, runner: number, orders: number } } = {};

        // Order counts per period
        const weeklyOrders: { [week: string]: { total: number, completed: number, pending: number, pendingPayment: number } } = {};
        const monthlyOrders: { [month: string]: { total: number, completed: number, pending: number, pendingPayment: number } } = {};
        const yearlyOrders: { [year: string]: { total: number, completed: number, pending: number, pendingPayment: number } } = {};

        // Initialize weekly orders
        Object.keys(weeklyRevenue).forEach(week => {
            weeklyOrders[week] = { total: 0, completed: 0, pending: 0, pendingPayment: 0 };
        });
        ...
        const aggregateOrders = (allOrders: any[]) => {
            allOrders.forEach(o => {
                ...
                if (!monthlyOrders[monthKey]) monthlyOrders[monthKey] = { total: 0, completed: 0, pending: 0, pendingPayment: 0 };
                ...
                if (!yearlyOrders[yearKey]) yearlyOrders[yearKey] = { total: 0, completed: 0, pending: 0, pendingPayment: 0 };
        ...
        runnerOrders.filter(o => o.itemsPaid && o.runnerFeePaid).forEach(o => {
            const date = new Date(o._creationTime);
            ...
            // Monthly
            const monthKey = `${date.getFullYear()}-${String(date.getMonth() + 1).padStart(2, '0')}`;
            if (!monthlyRevenue[monthKey]) monthlyRevenue[monthKey] = { total: 0, runner: 0, orders: 0 };
            monthlyRevenue[monthKey].total += revenue;
            monthlyRevenue[monthKey].runner += revenue;

            // Yearly
            const yearKey = String(date.getFullYear());
            if (!yearlyRevenue[yearKey]) yearlyRevenue[yearKey] = { total: 0, runner: 0, orders: 0 };
            yearlyRevenue[yearKey].total += revenue;
            yearlyRevenue[yearKey].runner += revenue;
        });

        // Aggregate Regular Orders Revenue
        orders.filter(o => o.itemsPaid && o.runnerFeePaid).forEach(o => {
            const date = new Date(o._creationTime);
