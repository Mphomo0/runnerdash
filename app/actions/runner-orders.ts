...
export async function getRunnerOrders(search?: string) {
  return await convex.query(api.runnerOrders.getRunnerOrders, search ? { search } : {});
}

export async function getRunnerOrder(id: string) {
  return await convex.query(api.runnerOrders.getRunnerOrder, { id: id as Id<"runnerOrders"> });
}
...
export async function updateRunnerOrder(id: string, data: {
  customerName?: string
  customerPhone?: string
  status?: string
  itemsPaid?: boolean
  ...
  await convex.mutation(api.runnerOrders.updateRunnerOrder, { id: id as Id<"runnerOrders">, ...data });
  revalidatePath('/runner-orders')
  revalidatePath(`/runner-orders/${id}`)
}

export async function deleteRunnerOrder(id: string) {
  await convex.mutation(api.runnerOrders.deleteRunnerOrder, { id: id as Id<"runnerOrders"> });
  revalidatePath('/runner-orders')
}
